...
class Generator:
  ...
  def __call__(self, input):
    ...
    self.reuse = True
    # 使用 tf1.get_collection 替代 tf.get_collection
    self.variables = tf1.get_collection(tf1.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)

    return output

  def sample(self, input):
    image = utils.batch_convert2int(self.__call__(input))
    # 使用 tf1.image.encode_jpeg 替代 tf.image.encode_jpeg
    image = tf1.image.encode_jpeg(tf1.squeeze(image, [0]))
    return image
